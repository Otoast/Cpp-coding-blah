...
Image::Image(const Image &objToCopy) : image(nullptr), width(0), height(0){
    allocateImage(objToCopy.width, objToCopy.height);
    for (unsigned int hei = 0; hei < objToCopy.height; ++hei){
        for (unsigned int wid = 0; wid < objToCopy.width; ++wid){
            (*this)[wid][hei] = objToCopy[wid][hei];
        }
    }
